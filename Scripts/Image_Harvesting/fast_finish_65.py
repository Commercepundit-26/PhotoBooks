...
QUERIES = [
    "romantic couple autumn park walking",
    "couple coffee date cafe smiling",
    "young couple piggyback laughing casual",
    "couple road trip mountains scenic",
    "couple laughing selfie camera smiling",
    "couple picnic park laughing blanket",
    "couple dancing kitchen dinner date",
    "couple winter coats snow laughing",
    "couple hugging street casual clothes",
...
with sync_playwright() as p:
    ...
    for q in QUERIES:
        ...
        for btn in btns:
            cur_count = len([f for f in os.listdir(TARGET_DIR) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
            if cur_count >= TARGET_TOTAL or got_in_q >= 4:
                break
