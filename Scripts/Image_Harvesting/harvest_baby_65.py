...
REQUIRED_KEYWORDS = ['baby', 'infant', 'newborn', 'toddler', 'child', 'kid', 'nursery', 'son', 'daughter', 'mother-and-baby', 'father-and-baby', 'parents']
...
SEARCH_TERMS = [
    ...
    "happy baby playing with wooden toys",
...
with sync_playwright() as p:
    ...
    for term in SEARCH_TERMS:
        ...
        for btn in btns:
            cur_count = len([f for f in os.listdir(TARGET_DIR) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
            if cur_count >= TARGET_TOTAL or got_in_term >= 5:
                break
