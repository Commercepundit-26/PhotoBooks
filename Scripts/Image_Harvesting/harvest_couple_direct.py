...
SEARCH_QUERIES = [
    ...
    "happy couple laughing piggyback outdoor casual",
    "young couple cooking kitchen dinner date",
    "couple road trip scenic viewpoint laughing",
    "couple picnic park casual clothes smiling",
    "couple coffee shop cafe smiling table",
    "couple walking city street winter coats holding hands",
    "couple laughing outdoor golden hour hug"
]
...
with sync_playwright() as p:
    ...
    for q in SEARCH_QUERIES:
        if len(os.listdir(TARGET_DIR)) >= TARGET_TOTAL:
            break

        print(f"\n>>> Searching: '{q}' (Currently {len(os.listdir(TARGET_DIR))}/{TARGET_TOTAL})")
            ...
            for btn in btns:
                ...
                try:
                    ...
                    if sugg not in seen_files and sugg.lower().endswith(('.jpg', '.jpeg', '.png')):
                        ...
                        print(f"  [{len(os.listdir(TARGET_DIR)):02d}/{TARGET_TOTAL}] ✓ [{orient}] {sugg} ({w}x{h} px, {mb:.1f} MB)")
                        time.sleep(0.3)
                except Exception:
                    pass
...
# Final audit
files = sorted([f for f in os.listdir(TARGET_DIR) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
total_mb = sum(os.path.getsize(os.path.join(TARGET_DIR, f)) for f in files) / (1024 * 1024)

print("\n" + "="*75)
print(f"COUPLE HARVEST COMPLETE: {len(files)} Master Photos ({total_mb:.1f} MB Total)")
print("="*75)
