...
# Clean out non-images or old files
for d in [TARGET_DIR, ALT_TARGET]:
    for f in os.listdir(d):
        fp = os.path.join(d, f)
        if os.path.isfile(fp):
            # keep if already a valid full-res jpg (> 1MB)
            if not f.lower().endswith(('.jpg', '.jpeg', '.png')) or os.path.getsize(fp) < 500000:
                os.remove(fp)
...
print("="*70)
print("ENVATO ELEMENTS: FULL-RES 60 MASTER PHOTO HARVESTER")
