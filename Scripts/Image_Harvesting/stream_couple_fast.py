...
with sync_playwright() as p:
    ...
    for q in QUERIES:
        ...
        print(f"\n[Progress: {cur_count:02d}/{TARGET_TOTAL}] -> Searching: '{q}'")
        page.goto("https://app.envato.com/photos", timeout=30000)
        time.sleep(2.0)
        ...
        for scroll_round in range(4):
            cur_count = len([f for f in os.listdir(TARGET_DIR) if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
            if cur_count >= TARGET_TOTAL or got_in_q >= 6:
                break
