...
print("="*70)
print("ENVATO ELEMENTS: TOPPING UP FULL-RES MASTER PHOTOS TO 60")
print("="*70)
...
with sync_playwright() as p:
    ...
    for orientation, target in TARGETS.items():
        ...
        print(f"\n>>> Checking {orientation}: currently have {have}/{target}")
        if have >= target:
            print(f"✓ Already at target for {orientation}.")
            continue

        page.goto("https://app.envato.com/", timeout=40000)
        time.sleep(2)

        # Search wedding ceremony / bride
        inp = page.wait_for_selector("input[placeholder*='Search']", timeout=10000)
        inp.fill(f"wedding {orientation.lower()}")
        inp.press("Enter")
        time.sleep(3)
        ...
        scrolls = 0
        while have < target and scrolls < 15:
            scrolls += 1
            ...
            for btn in dl_buttons:
                ...
                try:
                    ...
                    try:
                        ...
                        have += 1
                        print(f"  [{have}/{target}] ✓ {sugg_name} ({w}x{h} px, {mb:.1f} MB)")
                        time.sleep(0.8)
                    except Exception:
                        pass
                except Exception:
